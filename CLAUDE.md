...
**GL context lifetime ≠ process lifetime.** Native state is process-global, but the EGL context belongs to the Activity's `GLFrontBufferedRenderer`. A Back press finishes the Activity and leaves the process cached; the next launch gets a new context while every cached GL name (programs, VAOs, view FBOs, tile textures, text rasters, the thumbnail FBO) is stale, and `g_inited` made `ensureInited` skip relinking — symptom: blank pages until a hard kill. Two pieces fix it: `DrawingSurfaceView.announceGlContext` calls `onGlContextCreated` once per renderer instance from the first GL callback, and native `resetForNewGlContext` forgets every non-tile handle **without glDelete** (those numbers may already be the framework's buffers in the new context), relinks, then `rehydrateTilesAfterContextLoss` gives every tile new GL objects from its CPU mirror (`Tile::cachedBytes`) — no disk read, no free, no reallocation, so pages, layers, shapes, text boxes and the undo stack all survive. A tile whose mirror was nulled by a still-pending deferred save reverts to its on-disk state (rare: `release(false)` in `DrawingSurfaceView.release` lets onPause's drain pass run before teardown); a floating raster selection's pixels are lost but its source tiles are restored from the lift snapshots. Don't "simplify" this into drop-and-reload: freeing ~1100 mirrors costs ~3 s on the GL thread, and doing it on a background thread just slows the reload by the same amount. The announce first waits on a process-global latch that the previous renderer's `release(true) { }` callback counts down: `release` is asynchronous and the old GL thread can still be seconds into a background-page tile load; resetting under it segfaults (seen once). Any new GL handle cached in a global or a function-local static must be added to `resetForNewGlContext`.
...
- **Layout is zoom-independent, by construction.** `TextLayout.layout` always runs in doc px: text size = the box's font size, wrap width = `layoutWidthDoc` (box width rounded up to a whole px), paint flags = `kPaintFlags` (anti-alias + subpixel + **linear text**, i.e. no hinting, so advances scale exactly). Zoom only sets the resolution a raster is drawn at (`rasterize` draws that one layout through `canvas.scale`). The edit overlay uses the same paint flags and doc-px text size and is scaled as a View (`scaleX/Y`, pivot top-left), so its wraps and line heights are the raster's. The PDF path draws the same layout through the page scale. Don't reintroduce a layout at screen px anywhere — that was the bug where wraps and box heights drifted with zoom history. `uploadTextRaster` carries the layout width so the staleness test compares like with like. The overlay must measure unbounded (`OverlayEditText.onMeasure`) or the container clips it and the EditText scrolls to the caret, and it refuses finger `ACTION_DOWN` (`dispatchTouchEvent`) so two-finger gestures over the box reach the canvas beneath — the pen is the only thing that edits. Two things keep a resting raster looking like the overlay: `drawTextBoxes` pixel-snaps the quad (`uSnap`, level-0 sampling) whenever the texture is 1:1 with the screen and the box is axis-aligned, and `rasterize` draws the coverage in the box's colour (Skia's luminance-dependent glyph gamma) even though ALPHA_8 discards it. What remains — the raster is a stretched bitmap mid-gesture, re-drawn when the pinch settles — is deliberate: re-rastering a page-tall box costs tens of ms; GPU glyph rendering would be the real fix.
...
- **PDF page size**: the export dialog's `dpi` row (72 / 150 / 200 / 300 / custom, pref `export_dpi`, default 200 because the size presets are 200 dpi sizes) sets the PDF's physical page size: page = `px * 72 / dpi` points, the page canvas is scaled by that factor, and the bitmap is still embedded at full doc-px resolution. The readout under the row shows the resulting in / mm size. PNGs carry no DPI metadata.
